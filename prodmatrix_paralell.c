...
void matrix_product();
void *matrix_prod_by_slices( void *thid);
...
char *msg[15] = { \
	"<matrix1> <matrix2> <matrix_output> <size> <number_of_threads>", \
...
// the first and the last elements to take for each thread
long **a_limits;
...
// number of threads, defined by command line parameter
long NTH;
...
int main(int argc, char *argv[]){
	...
	switch(argc){
		case 6:
			// set matrix size and number of threads by command line parameter
			sscanf(argv[4], "%ld", &N);
			sscanf(argv[5], "%ld", &NTH);
			...
			if (NTH<1 || NTH>8){
				error += _MSG_ERR_NTHSIZE;
				printf("%s %s\n", msg[_MSG_ERR_GENERR], msg[_MSG_ERR_NTHSIZE]);
			}
...
void matrix_product(){
	...
	// initialize a_limits
	for(i=0; i<NTH; i++){
		a_limits[i][0] = (N/NTH)*i;
		a_limits[i][1] = (N/NTH)*(1+i) - 1;
	}
	// avoid losing the last elements if N is not divisible by NTH
	a_limits[NTH-1][1] = N-1;
	
	// create each thread
	for(i=0; i<NTH; i++){
		ret = pthread_create(&th[i], NULL, matrix_prod_by_slices, (void *) i);
	}
...
void *matrix_prod_by_slices(void *thid){
 	int i, j, k;
	int f, l;
	f = a_limits[(long) thid][0];
	l = a_limits[(long) thid][1];
